	...
	public partial class blendr : Form
	{
	    ...
	    SqlDataReader read= command.ExecuteReader();
			    ...
			    while (read.Read())
			    {
			        ...
			        if(read["Gumitömítés"] == DBNull.Value){
			        	checkBox14.Checked = false;
			        }
			        else{
			        checkBox14.Checked = (bool)read["Gumitömítés"];			        	
			        }
			        if(read["Betoltocso"] == DBNull.Value){
			        	checkBox15.Checked = false;
			        }
			        else{
			        checkBox15.Checked = (bool)read["Betoltocso"];			        	
			        }
			    }
			    read.Close();
			}
		...
		void Button1Click(object sender, EventArgs e)
		{
			...
			Szivarogepor = @Szivarogepor, IBCbatch = @IBCbatch, Szivaroge = @Szivaroge, Komment = @Komment, Datum = @Datum, Ellenorzo = @Ellenorzo, Ellenorizve = @Ellenorizve, Ki = @Ki, Felrazvahoe = @Felrazvahoe, Jerrycane = @Jerrycane, Muszakie = @Muszakie, Idegene = @Idegene, Gumitömítés = @Gumitömítés, Betoltocso = @Betoltocso 
			...
			cmd.Parameters.Add(new SqlParameter("@Gumitömítés", checkBox14.Checked));	
			cmd.Parameters.Add(new SqlParameter("@Betoltocso", checkBox15.Checked));			
			cmd.ExecuteNonQuery();
			conn.Close();
